...
#data csv file name
data_fname = 'data2.csv'

#use Model 1 - use A2, A3, A4, A5
loaded_model = pickle.load(open('DT_player_clustering32.sav', 'rb'))
classdict = {0: 'Casual Achiever', 1: 'Hardcore Killer', 2: 'Casual Killer', 3: 'Hardcore Achiever'}
...
def show_score(score, level, font_size = 16, x=10, y=10):
	...
	player_text = score_font.render("Name : " + str(PLAYER_NAME), True, (255, 255, 0))	
	...
	if time_sec != 0:
		player_class_text = score_font.render("Player_Type : " + str(classdict[int(loaded_model.predict([[coin_collected, enemy_killed, bullet_count, bullet_missed]]))]), True, (255, 255, 0)) 	#Class


	screen.blit(player_text, (x, y))	
	screen.blit(level_text, (x, y + 20))
	screen.blit(score_text, (x, y + 25 + font_size))

	screen.blit(show_axisX, (x, y + 25 + (font_size*6) + font_size))			#A0
	screen.blit(show_axisY, (x, y + 25 + (font_size*7) + font_size))			#A1

	screen.blit(coin_collected_text, (x, y + 25 + (font_size*1) + font_size))	#A2
	screen.blit(enemy_killed_text, (x, y + 25 + (font_size*2) + font_size))		#A3
	screen.blit(bullet_count_text, (x, y + 25 + (font_size*3) + font_size))		#A4
	screen.blit(bullet_missed_text, (x, y + 25 + (font_size*4) + font_size))	#A5
	if time_sec != 0:
		screen.blit(player_class_text, (x, y + 25 + (font_size*5) + font_size))		#Class
	...
	# 	RANK_4_text = score_font.render(str(RANK_ONLINE_PLAYER[3]), True, (255, 255, 0))
	# 	screen.blit(RANK_4_text, ( int(screen_sizeX - 100 - (len(RANK_ONLINE_PLAYER[3])*2)) , y + 5 + font_size*4 ))	# 4
	# if len(RANK_ONLINE_PLAYER) > 4 :
	# 	RANK_5_text = score_font.render(str(RANK_ONLINE_PLAYER[4]), True, (255, 255, 0))
	# 	screen.blit(RANK_5_text, ( int(screen_sizeX - 100 - (len(RANK_ONLINE_PLAYER[4])*2)) , y + 5 + font_size*5 ))	# 5
...
def show_game_over(screen_sizeX, screen_sizeY, score, high_score):
	...
	message_display_center('Your Score              : ' + str(score), font_medium, yellow, int(screen_sizeX/2), int(screen_sizeY *14/20))
	message_display_center('Session Top Score : ' + str(high_score), font_medium, yellow, int(screen_sizeX/2), int(screen_sizeY *15/20))
	message_display_center('Your Player Type : ' + str(str(classdict[int(loaded_model.predict([[coin_collected, enemy_killed, bullet_count, bullet_missed]]))])), font_medium, yellow, int(screen_sizeX/2), int(screen_sizeY *16/20))
...
def updateOnlineScore(data) :
	...
	global scoredict
	for i in ONLINE_PLAYER_SORTED :
		if i[0] not in scoredict:	#if name not exist, add to dict
			scoredict[i[0]] = i[1]
		if i[1] > scoredict[i[0]]:	#if there is new highscore for 'name', update score
			scoredict[i[0]] = i[1]

	# online_score = sorted(scoredict.items(), key=lambda x: x[1], reverse=True)[:3]	#only top3
	online_score = sorted(scoredict.items(), key=lambda x: x[1], reverse=True)		#every player

	for i in online_score :
		RANK_ONLINE_PLAYER.append(f'{i[0]} : {i[1]}')
...
while not quit_game:
	...
	if os.path.exists("data2.csv") == False:
		pd.DataFrame(columns=['level', 'score', 'name', 'time', 'A0', 'A1', 'A2', 'A3', 'A4', 'A5']).to_csv(data_fname, index=False)
	...
	while not go_to_menu and not quit_game:
		...
		if level_iter > level_change and not game_over:
			...
			# increase number of coins with higher speed
			# coin_image_index = (level -1) % len(coin_image)
...
#			score += level_score_increase
		...
		for event in pygame.event.get():	
			...
			if event.type == pygame.USEREVENT and not game_over : #netpiesent
				...
				time_sec = time_sec +1 
				data = f'"name" : "{name}", "X" : {player.posX}, "Y" : {player.posY} , "score" : {score}, "coin" : {coin_collected}, "killed" : {enemy_killed}, "shots" : {bullet_count}, "miss" : {bullet_missed}, "time" : {time_sec}'
				...
				if time_sec%1 == 0:
					...
					'A0': str(player.posX),
					'A1': str(player.posY),
					'A2': str(coin_collected),
					'A3': str(enemy_killed),
					'A4': str(bullet_count),
					'A5': str(bullet_missed)
					}, ignore_index=True)
